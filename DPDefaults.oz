...
define
   ...
   fun {ComposeURI IP PN ID}
      % An IPv6 address, add square brackets around it [rfc2732].
      IPString = if {Member &: {VirtualString.toString IP}} then
         '['#IP#']'
      else
         IP
      end
   in
      {VirtualString.toString 'oz-site://'#IPString#':'#PN#'/'#ID}
   end

   fun {DecomposeURI URI}
      IP PN ID
      UriWithoutScheme = {List.drop URI PrefixLen}
      Rest
   in
      if UriWithoutScheme.1 == &[ then
         {String.token UriWithoutScheme.2 &] IP Rest}
         {String.token Rest.2 &/ PN ID}
      else
         {String.token UriWithoutScheme &: IP Rest}
         {String.token Rest &/ PN ID}
      end
      IP#PN#ID
   end
